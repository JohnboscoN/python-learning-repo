...
from config_903 import DateCols903
from config_903 import DateCols903, EthnicSubcategories
import numpy as np
#from utils import multiples_same_event
...
def clean_903_table(df: pd.DataFrame, collection_end: pd.Timestamp):
    """
    this function takes and cleans 903 tables, it will be used in the 903 pipeline to clean all tables in the database. It will:
- drop the index column if it exists
    ...
    if "DOB_dt" in clean_df.columns:
        ...
        clean_df['AGE_BUCKET'] = clean_df['AGE'].apply(calculate_age_buckets)
...
def time_difference(start_col, end_col,business_days=False):
    ''' A function to calculate time difference between two date columns, with option to calculate business days.`
'''

    if business_days:
        #np.busday_count can only use datetime64 dtype, so we need to convert the objects
            time_diff = np.busday_count(
                start_col.values.astype('datetime64[D]'),
                end_col.astype('datetime64[D]')
            )
...
#     multiples = df.groupby("CHILD").size().to_frame("No of events").reset_index()
#     multiples =multiples.groupby("No of events").size().to_frame("Children with number of events").reset_index()
...
#  def group_calculation_year(df, column, measure_name):
#     '''
#     A function to group as df by input column, outputs with count
#     and percentage to a dataframe with renamed columns.
#     '''
#     grouped = df.groupby(column).size()
#     grouped = grouped.to_frame(f'{measure_name} - Count').reset_index()
#     grouped = grouped.rename(columns={column:'Year'})
...
#         merged_df["on both"] = "Yes"

#         df = df1.merge(merged_df[['CHILD', 'on both']], on='CHILD', how='left')

#         df.fillna(("on both", "No"), inplace=True)
...
def percent_of_col_with_value(df, col, measure_name):
    ...
    grouped = group_calculation(df, col, measure_name)
